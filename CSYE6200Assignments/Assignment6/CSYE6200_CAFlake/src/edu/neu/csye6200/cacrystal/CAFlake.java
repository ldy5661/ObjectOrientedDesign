...
//                    System.out.print(BRIGHT_BLACK + " " + ANSI_RESET + " "); //"□"
    ...
    public void displayFlakeDebug(CAFlake flake) {
        System.out.println("new Flake:");
        for (int i = 0; i < flake.getCells().length; i++) {
            if (i == 3 || i == 5) {
                System.out.print("SUO");
            }
            for (int j = 0; j < flake.getCells()[0].length; j++) {                
                if (flake.getCells()[i][j].getFilled() == 0) {
                    //System.out.print(BACKGROUND_BRIGHT_BLUE + "   " + ANSI_RESET);
//                    System.out.print(BRIGHT_BLACK + " " + ANSI_RESET + " "); //"□"
    ...
    public void displayFlakeTABB(CAFlake flake) {
        int cellSize = flake.getCells().length;
        int mid = cellSize / 2;
        int space = 0;
        for (int i = 0; i < cellSize; i++) {
            ...
            if (i % (cellSize - 1) == 0) {
                System.out.print(String.format("%" + mid +"s", " "));
            }
            for (int j = 0; j < flake.getCells()[0].length; j++) {                
                if (flake.getCells()[i][j].getFilled() == 0) {
                    System.out.print(BACKGROUND_BRIGHT_BLUE + "   " + ANSI_RESET);
